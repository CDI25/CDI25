...
if __name__ == '__main__':
    ...
    org_miss_postion = np.where(mask_flag == 1)

    mask_evalset = np.random.rand(mask_flag.shape[0], mask_flag.shape[1])
    mask_evalset[np.where(mask_evalset >= 0.1)] = 0
    mask_evalset[np.where(mask_evalset != 0 )] = 1
    mask_evalset[:,2:] = 0
    mask_evalset[org_miss_postion] = 1


    norm_data, min_sensor_p, max_sensor_p = data_preprocess.normalization(org_data, mask_evalset)
    init_data = data_preprocess.EWMA_init(norm_data, mask_evalset)
    # init_data = data_preprocess.Interpolation_init(norm_data, mask_evalset)
    print("init ready")
    ...
    regression_list = utils.Regression_models(args, norm_data, mask_evalset).regression_list
    

    cmdi_data = utils.CMDI_DATA(args, norm_data, mask_evalset)
    cmdi_model = utils.CMDI(args, init_data, mask_evalset)
    ...
    mask_evalset[org_miss_postion] = 0
    eval_result = test(args, cmdi_model, min_sensor_p, max_sensor_p, org_data, mask_evalset)
