...
from train_stream import test, train
...
if __name__ == '__main__':
    ...
    data_x, data_y = norm_data.shape

    init_data = data_preprocess.Interpolation_init(norm_data[:data_x // 2], mask_flag[: data_x // 2])
    print("init ready")


    regression_list = utils.Regression_models(args, norm_data, mask_flag).regression_list

    for iter in range(data_x // 2, data_x-1):
        operate_data = norm_data[iter-1:iter+2]
        operate_mask = mask_flag[iter-1:iter+2]
        if np.sum(operate_mask[1]) == 0:
            continue
        ...
        cmdi_data = utils.CMDI_DATA(args, operate_data, operate_mask)
        cmdi_model = utils.CMDI(args, operate_data, operate_mask)
        cmdi_model.train()
        ...
        train(args, cmdi_model, regression_list, norm_data, cmdi_data.input_of_processor[0])
        ...
        eval_result = test(args, cmdi_model, min_sensor_p, max_sensor_p, operate_data, operate_mask)
        eval_result = eval_result[1]

        truth = org_data[iter]
        eval_mask = mask_flag[iter]

        f = open("time_stream.txt","a")
        print(training_cost,file=f)

        result_all.append(eval_result)
        truth_all.append(truth)
        eval_mask_all.append(eval_mask)


result_all = np.array(result_all)
truth_all = np.array(truth_all)
eval_mask_all = np.array(eval_mask_all)


res = np.sqrt(((result_all[np.where(eval_mask_all == 1)] - truth_all[np.where(eval_mask_all == 1)]) ** 2).mean())
print(res)
